...
public class LoyaltyPointController {
    ...
    public ResponseEntity<ApiResponse<List<LoyaltyPointResponse>>> getCustomerHistory(@PathVariable String customerId) {
        List<LoyaltyPointResponse> history = loyaltyPointService.getHistoryByCustomerId(customerId);
        return ResponseEntity.ok(new ApiResponse<>(200, "Lấy lịch sử điểm tích lũy của khách hàng thành công!", history));
    }
}