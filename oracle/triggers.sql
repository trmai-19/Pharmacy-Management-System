...
BEGIN
    ...
    -- 2. Cập nhật GIABAN = GIANHAP * 1.2 
    -- NHƯNG CHỈ KHI Admin chưa set giá (GIABAN đang là NULL hoặc bằng 0)
    IF :NEW.GIANHAP > 0 THEN
        UPDATE SANPHAM 
        SET GIABAN = ROUND(:NEW.GIANHAP * 1.2, 0)
        WHERE MASP = v_masp AND (GIABAN IS NULL OR GIABAN = 0);
    END IF;
END;
/
...
--SEQUENCES -> trong file database.sql nhe
